...
public:
	...
	Gauss(int n) : Matrix <T>(n){
		...
		this->rightVectorTemp = new double[this->size];

		for (int i = 0; i < this->size; i++) {
			cout << "Введите " << i + 1 << " элемент правой части:" << endl;
			cin >> rightVector[i];
			rightVectorTemp[i] = rightVector[i];
		}
	}
	...
	void check(){
		...
		for (int i = 0; i < this->size; i++){
			...
			cout << rightVector[i] << "\t";
			if (rightVector[i] != rightVectorTemp[i]) {
				cout << endl << "Система не совместна - решений не имеет" << endl;
				exit(2);
			}
		}
	}
